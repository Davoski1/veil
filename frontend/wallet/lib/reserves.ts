 ...
 * liabilities on the DEX. Ignoring that is invisible on testnet, where a
 * Friendbot account holds 10,000 XLM and the reserve is noise — but on mainnet,
 * where balances are small and real, a "Max" that offers the full balance
 * builds a transaction that cannot succeed (tx_insufficient_balance), and the
 * user only finds out after signing.
 *
